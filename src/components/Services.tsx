    ...
    <section id="services" className="py-20 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-lg mt-6 text-muted-foreground italic text-[#4d4d4d] max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card key={index} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border-border bg-card">
              ...
              <CardContent>
                <CardDescription className="text-muted-foreground text-[#4d4d4d]">{service.description}</CardDescription>
              </CardContent>
            </Card>
          ))}
        </div>
